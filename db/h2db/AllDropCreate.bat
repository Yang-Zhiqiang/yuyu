@echo off
  ...
  echo 「%DDL_DIR%」配下のDDLファイル群で全てのテーブルをDROP/CREATEします。
  echo ------------------------------------------------------------------------------------------
  set /P YN="実行しますか？ (y) / n ："
  echo.
...
:mainProc
  ...
  for %%j in (%H2JAR%) do (
    echo.
    echo ----------------------------------------------------------------------
    echo  全テーブル・ビューを drop / create するSQLスクリプトを生成しています。
    echo ----------------------------------------------------------------------
    (
      for %%f in (%DDL_VIEW_DIR%\*.sql) do (
        echo drop view if exists %%~nf;
      )
      for %%f in (%DDL_DIR%\*.sql) do (
        echo drop table if exists %%~nf;
        type %%f
      )
      for %%f in (%DDL_VIEW_DIR%\*.sql) do (
        type %%f
