...
(SELECT
    S.syono
FROM
    JT_SkKihon_Z SKK JOIN JT_Sk_Z S ON SKK.seikyuusyubetu = '2' AND SKK.syono = S.syono AND  SKK.skno = S.skno
WHERE
    A.syono = S.syono
)
) C
LEFT OUTER JOIN IT_KykSyouhn_Z D
ON C.syono = D.syono AND D.syutkkbn = '1'
LEFT OUTER JOIN IT_KykKihon_Z E
ON C.syono = E.syono
LEFT OUTER JOIN IT_KykUkt_Z F
ON C.syono = F.syono AND F.uktsyukbn = '1' AND F.uktsyurenno = 1
LEFT OUTER JOIN IT_KykSonotaTkyk_Z G
ON C.syono = G.syono
LEFT OUTER JOIN IT_KhTtdkTyuui_Z H
ON C.syono = H.syono
UNION ALL
/* 受付中、支払済、診断書料支払 */
SELECT 
...
LEFT OUTER JOIN JT_SiRireki_Z D 
ON C.syono = D.syono AND C.skno = D.skno
LEFT OUTER JOIN IT_KykSyouhn_Z E
ON C.syono = E.syono AND E.syutkkbn = '1'
LEFT OUTER JOIN IT_KykKihon_Z F
ON C.syono = F.syono
LEFT OUTER JOIN IT_KykUkt_Z G
ON C.syono = G.syono AND G.uktsyukbn = '1' AND G.uktsyurenno = 1
LEFT OUTER JOIN IT_KykSonotaTkyk_Z H
ON C.syono = H.syono
LEFT OUTER JOIN JT_SkJiyuu_Z I
ON C.syono = I.syono AND C.skno = I.skno AND C.seikyuurirekino = I.seikyuurirekino
LEFT OUTER JOIN IT_KhTtdkTyuui_Z J
ON C.syono = J.syono
;