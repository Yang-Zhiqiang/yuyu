...
FROM (
 ...
 SELECT
  ...
  TEISEI2.KARIKATATEISEIGK$
 FROM
  BT_HJYBRUIKEIGAKUTEISEI AS TEISEI2
 LEFT JOIN (
  SELECT
   HJYBTANTOSITUCD,
   KANJYOUKMKCD,
   ...
   KASIKATARUIGK$
  FROM
   BT_HJYBRUIKEIGAKU AS T3
  WHERE T3.DENYM = (
   SELECT MAX(DENYM) FROM BT_HJYBRUIKEIGAKU AS T4
...
LEFT JOIN BM_KANJYOUKAMOKU AS KAMOKU
