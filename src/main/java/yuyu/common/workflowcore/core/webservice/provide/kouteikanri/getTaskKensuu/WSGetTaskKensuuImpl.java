...
import yuyu.common.workflowcore.core.webservice.provide.kouteikanri.getTaskKensuu.IWSGetTaskKensuuIOKey.MapKey;
import yuyu.common.workflowcore.iwfcommon.CheckException;
import yuyu.def.classification.C_ResultCdKbn;
import yuyu.def.workflowcore.manager.IwfCoreDomManager;

/**
 * タスク件数照会のWebサービスのメインクラスです。<br />
 */
public class WSGetTaskKensuuImpl  implements WSGetTaskKensuu {

    final static Logger LOGGER = LoggerFactory.getLogger(WSGetTaskKensuu.class);

    @Inject
    private IwfCoreDomManager iwfCoreDomManager;


    public WSGetTaskKensuuImpl() {
        super();
    }


    public WSGetTaskKensuuResponse validate(WSGetTaskKensuuRequest poRequest) throws Exception {

        LOGGER.debug("タスク件数照会│バリデーションチェック処理を開始します。");

        WSGetTaskKensuuResponse poResponse = new WSGetTaskKensuuResponse();
    ...
    @Override
    public WSGetTaskKensuuResponse executeBizMain(WSGetTaskKensuuRequest poRequest) throws Exception {

        WSGetTaskKensuuResponse oWSResponse = new WSGetTaskKensuuResponse();
        ...
        try {
            List<Object[]> taskCountSummaryList = iwfCoreDomManager.getTaskKensuuByCond(sAccountid, sWariateumu,
                sTaskendumu,
                sFlowid, sTaskid,
            ...
            if (taskCountSummaryList.isEmpty()) {
                ...
                LOGGER.debug("タスク件数照会データが存在しません。");
            ...
            for (Object[] taskCountSummary : taskCountSummaryList) {
                ...
                summaryMap.put(MapKey.sS_TASK_COUNT, taskCountSummary[4].toString());

                taskSummaryMaps.add(summaryMap);
            }

            oWSResponse.setTaskSummaryMaps(taskSummaryMaps.toArray(new Map[] {}));
            IwfEditMessageUtil.setMsgResultOK(oWSResponse, "タスク件数照会サービス");
            ...
            IwfEditMessageUtil.setMsgResultNG(oWSResponse, IwfMessageCd.IWF0001, "タスク件数照会");
        } finally {
    ...
    public WSGetTaskKensuuResponse getBootErrResponse() {
        WSGetTaskKensuuResponse res = new WSGetTaskKensuuResponse();
