 ...
 * 新契約 工程 新契約工程遷移（成立）
 */
public class SkProcessForwardSeiritu {
    ...
    private static final String KINOUID_HENKINKIBOU = "skhenkinkibou";

    private static final String KINOUID_HENKINNYUURYOKU = "skhenkin";

    private static final String KINOUID_FBHENKINTORIKESI = "sktoujitufbhenkintorikesi";

    public SkProcessForwardSeiritu() {
        super();
    }

    public void exec(SkProcessForwardInBean pSkProcessForwardInBean) {

        logger.debug("▽ 新契約工程遷移（成立） 開始");
        ...
        if (bzGetProcessSummaryOutBeanList != null && 0 < bzGetProcessSummaryOutBeanList.size()) {

            bzGetProcessSummaryOutBean = bzGetProcessSummaryOutBeanList.get(0);

            if (ISkCommonKoumoku.NODEID_KOUTEIKANRYOU.equals(bzGetProcessSummaryOutBean.getNowNodoId()) &&
                (KINOUID_HENKINKIBOU.equals(kinou.getKinouId()) ||
                    KINOUID_HENKINNYUURYOKU.equals(kinou.getKinouId()) ||
                    KINOUID_FBHENKINTORIKESI.equals(kinou.getKinouId()))) {

                logger.debug("△ 新契約工程遷移（成立） 終了");
        ...
        bzGetProcessSummaryOutBeanList = bzGetProcessSummary
            .exec(bzGetProcessSummaryInBean);

        if (0 < bzGetProcessSummaryOutBeanList.size()) {
        ...
        bzOutputProcessRecord.exec(bzGetProcessSummaryOutBean.getKouteiKanriId(),
            ...
            MessageUtil.getMessage(MessageId.IHC0001),
