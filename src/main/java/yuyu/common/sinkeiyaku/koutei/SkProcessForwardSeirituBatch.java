...
/**
 * 新契約 工程 新契約工程遷移（成立）_バッチ用
 */
public class SkProcessForwardSeirituBatch {

    @Inject
    private static Logger logger;

    public SkProcessForwardSeirituBatch() {
        super();
    }

    public List<BzProcessForwardForBatchInBean> exec(SkProcessForwardForBatchInBean pSkProcessForwardForBatchInBean) {

        logger.debug("▽ 新契約工程遷移（成立）_バッチ用 開始");
        ...
        if (bzGetProcessSummaryOutBeanList != null
            && bzGetProcessSummaryOutBeanList.size() > 0) {
        ...
        bzProcessForwardForBatchInBean.setSyoriComment(MessageUtil.getMessage(MessageId.IHC0001));
        ...
        skInputKokutiRuleBean.setMosuketukeKbn(C_MosUketukeKbn.getContentByValue(
            ...
            String.valueOf(pSkProcessForwardForBatchInBean.getHtSyoriCTL().getMosuketukekbn())));
        ...
        if (bzGetProcessSummaryOutBeanList.size() > 0) {
            ...
            skHubiKaisyouRuleBean.setHubikaisyoujoutaiKbn(C_HubikaisyoujoutaiKbn.getContentByValue(
                ...
                String.valueOf(pSkProcessForwardForBatchInBean.getHtSyoriCTL().getHubikaisyoujoutaikbn())));
        ...
        if (bzGetProcessSummaryOutBeanList.size() > 0) {
            ...
            skKeiyakukakuninRuleBean.setKykkakjkKbn(C_KykkakjkKbn.getContentByValue(
                ...
                String.valueOf(pSkProcessForwardForBatchInBean.getHtSyoriCTL().getKykkakjkkbn())));
        ...
        logger.debug("△ 新契約工程遷移（成立）_バッチ用 終了");
