...
/**
 * 契約保全工程ワークリスト のビジネスロジックです。
 */
public class HozenWorkListBizLogic {

    @Inject
    private static Logger logger;

    @Inject
    private HozenWorkListUiBean uiBean;

    @Inject
    private HozenWorkListSearchKeyInputBean searchKeyInputBean;
    ...
    void init() {
        ...
        String[] pSubSystemId = {HozenWorkListConstants.HOZEN};
    ...
    String taskSntkNyuuryokuLinkOnClick(C_TaskKbn pTasksentakujoutai) {
        ...
        if(!HozenWorkListConstants.HOZEN_KINOUIDMAP.containsKey(jimutetuzukiCd + taskId)){
            throw new BizAppException(MessageId.EAS0037);
        }
        if(HozenWorkListConstants.HOZEN_KINOUIDMAP.get(jimutetuzukiCd + taskId) == null){
        ...
        return HozenWorkListConstants.HOZEN_KINOUIDMAP.get(key);
    ...
    String itiranLinkOnClick(C_TaskKbn pTasksentakujoutai) {
        ...
        return HozenWorkListConstants.FORWARDNAME_INPUTKEYANKNITRNKYKHZN;
    ...
    String anknItrnNyuuryokuLinkOnClick() {
        ...
        if(!HozenWorkListConstants.HOZEN_KINOUIDMAP.containsKey(tempJimuTetuzukiCd + tempTaskId)){
            throw new BizAppException(MessageId.EAS0037);
        }
        if(HozenWorkListConstants.HOZEN_KINOUIDMAP.get(tempJimuTetuzukiCd + tempTaskId) == null){
            throw new BizLogicException(MessageId.EDF1001);
        }

        return HozenWorkListConstants.HOZEN_KINOUIDMAP.get(tempJimuTetuzukiCd + tempTaskId);
    ...
    void modorubtnOnClick() {
        ...
        uiBean.setPageNo(HozenWorkListConstants.PAGENO_INPUTKEYTSKSNTKKYKHZN);
    ...
    private void getSearchResult(DT_WfSentakujyotaiHozon pSearchCond){
        ...
        if(pageNo == HozenWorkListConstants.PAGENO_INPUTKEYTSKSNTKKYKHZN){
    ...
    private GetTasksForWorkListResultBean getTasksForWorkList(){
        ...
        if(uiBean.getPageNo() == HozenWorkListConstants.PAGENO_INPUTKEYTSKSNTKKYKHZN){
        ...
        if(searchresult.getExecutableTaskInfoBeanList().size() > 0){
            for (ExecutableTaskInfoBean tempresult:searchresult.getExecutableTaskInfoBeanList()) {
                ...
                if(tempresult.getPersonalizedTaskNum().compareTo(BizNumber.valueOf(0)) > 0){
                    taskResultInfoDataSourceBean.setKojinNyuuryokuLink(HozenWorkListConstants.NYUURYOKU_LINK);
                    taskResultInfoDataSourceBean.setKojinItiranLink(HozenWorkListConstants.ITIRAN_LINK);
                }
                ...
                if(tempresult.getSharedTaskNum().compareTo(BizNumber.valueOf(0)) > 0){
                    taskResultInfoDataSourceBean.setKyouyouNyuuryokuLink(HozenWorkListConstants.NYUURYOKU_LINK);
                    taskResultInfoDataSourceBean.setKyouyouItiranLink(HozenWorkListConstants.ITIRAN_LINK);
                }
    ...
    private DT_WfSentakujyotaiHozon getSearchCond(String wkUserId) {
        DT_WfSentakujyotaiHozon searchCond = workflowDomManager.getWfSentakujyotaiHozon(wkUserId, HozenWorkListConstants.HOZEN);
        ...
        if(jimustartymdfrom != null && jimustartymdto != null){
            if( jimustartymdfrom.intValue() > jimustartymdto.intValue() ){
                throw new BizLogicException(MessageId.EDF0003,
                    new Item[]{GenHozenWorkListConstants.DDID_SEARCHKEYINPUT_JIMUSTARTYMDFROM,
                    GenHozenWorkListConstants.DDID_SEARCHKEYINPUT_JIMUSTARTYMDTO},
                    "事務開始日" );
            }
        }
    }
    ...
    private void checkInputJimucode() {
        ...
        if( (!"".equals(jimuyoucd)) && commonCheckBox ){
            throw new BizLogicException(MessageId.EDF0006,
                GenHozenWorkListConstants.DDID_SEARCHKEYINPUT_JIMUYOUCD,
                GenHozenWorkListConstants.DDID_SEARCHKEYINPUT_JMYCDMISETTEIKBN );
        }
    }

    private void saveSearchCond() {

        DT_WfSentakujyotaiHozon searchCond = workflowDomManager.getWfSentakujyotaiHozon(baseUserInfo.getUserId(), HozenWorkListConstants.HOZEN);

        if(searchCond == null){
            searchCond = new DT_WfSentakujyotaiHozon();

            searchCond.setUserId(baseUserInfo.getUserId());
            searchCond.setSubSystemId(HozenWorkListConstants.HOZEN);
    ...
    private List<ResultInfoItiranDataSourceBean> getViewValues(List<TaskSummaryForWorkListBean> pList) {
        ...
        List<WorklistViewValuesBean> retList = WorklistViewValuesCreator.createValues(
            HozenWorkListConstants.HOZEN, wkList);
        ...
        for(int cnt = 0;cnt < retList.size();cnt++){
            ...
            if(pList.get(cnt).getNyuuryokuKahi()){
                tempItiranBean.setNyuuryokuLink(HozenWorkListConstants.NYUURYOKU_LINK);
            }
