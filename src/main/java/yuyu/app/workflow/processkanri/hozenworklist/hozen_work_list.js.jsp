<%--
    hozen_work_list.js.jsp - 契約保全工程ワークリスト JavaScript(JSP)
--%>
<%@page language="java" contentType="text/javascript; charset=UTF-8" pageEncoding="UTF-8" %>
<%@page import="static jp.co.slcs.swak.web.struts.ui.IdSelectorConstants.*" %>
<%@page import="static yuyu.app.workflow.processkanri.hozenworklist.HozenWorkListConstants.*" %>
<%@page import="static yuyu.app.workflow.processkanri.hozenworklist.HozenWorkListSelector.*"  %>
<%@page import="jp.co.slcs.swak.core.inject.SWAKInjector" %>
<%@page import="yuyu.app.workflow.processkanri.hozenworklist.HozenWorkListUiBean" %>
<% HozenWorkListUiBean uiBean = SWAKInjector.getInstance(HozenWorkListUiBean.class); %>
<script type="text/javascript"><!--
$(function(){

  $(".messagesItem").each(
  function(){
    <% if(uiBean.getPageNo() == PAGENO_INPUTKEYTSKSNTKKYKHZN){ %>
        if ($(<%= RESULTINFOTSKSNTKBASE %>)[0].rows.length <= 1) {
            $(<%= RESULTINFOTSKSNTKBASE %>).visible(false);
        }
    <% } %>

    <% if(uiBean.getPageNo() == PAGENO_INPUTKEYANKNITRNKYKHZN){ %>
