<%--
    nenkin_work_list.js.jsp - 年金支払工程ワークリスト JavaScript(JSP)
--%>
<%@page language="java" contentType="text/javascript; charset=UTF-8" pageEncoding="UTF-8" %>
<%@page import="static jp.co.slcs.swak.web.struts.ui.IdSelectorConstants.*" %>
<%@page import="static yuyu.app.workflow.processkanri.nenkinworklist.NenkinWorkListConstants.*" %>
<%@page import="static yuyu.app.workflow.processkanri.nenkinworklist.NenkinWorkListSelector.*"  %>
<%@page import="jp.co.slcs.swak.core.inject.SWAKInjector" %>
<%@page import="yuyu.app.workflow.processkanri.nenkinworklist.NenkinWorkListUiBean" %>
<% NenkinWorkListUiBean uiBean = SWAKInjector.getInstance(NenkinWorkListUiBean.class); %>
<script type="text/javascript"><!--
$(function(){
  $(".messagesItem").each(
  function(){
    <% if(uiBean.getPageNo() == PAGENO_INPUTKEYTSKSNTKNKSHR){ %>
        if ($(<%= RESULTINFOTSKSNTKBASE %>)[0].rows.length <= 1) {
            $(<%= RESULTINFOTSKSNTKBASE %>).visible(false);
        }
    <% } %>

    <% if(uiBean.getPageNo() == PAGENO_INPUTKEYANKNITRNNKSHR){ %>
    ...
    $(<%= CLEARBTN_SEARCHCONDITIONBTN %>).click(
      function(){
        ...
        $(<%= JIMUSTARTYMDFROM_SEARCHKEYINPUT_GENGOU %>).val("H");
        $(<%= JIMUSTARTYMDTO_SEARCHKEYINPUT_GENGOU %>).val("H");
        $(<%= JIMUSTARTYMDFROM_SEARCHKEYINPUT_DATE %>).val("");
        $(<%= JIMUSTARTYMDTO_SEARCHKEYINPUT_DATE %>).val("");
      }
    );
