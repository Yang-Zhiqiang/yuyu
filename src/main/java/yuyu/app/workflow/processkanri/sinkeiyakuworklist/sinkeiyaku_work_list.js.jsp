<%--
    sinkeiyaku_work_list.js.jsp - 新契約工程ワークリスト JavaScript(JSP)
--%>
<%@page language="java" contentType="text/javascript; charset=UTF-8" pageEncoding="UTF-8" %>
<%@page import="static jp.co.slcs.swak.web.struts.ui.IdSelectorConstants.*" %>
<%@page import="static yuyu.app.workflow.processkanri.sinkeiyakuworklist.SinkeiyakuWorkListConstants.*" %>
<%@page import="static yuyu.app.workflow.processkanri.sinkeiyakuworklist.SinkeiyakuWorkListSelector.*"  %>
<%@page import="jp.co.slcs.swak.core.inject.SWAKInjector" %>
<%@page import="yuyu.app.workflow.processkanri.sinkeiyakuworklist.SinkeiyakuWorkListUiBean" %>
<% SinkeiyakuWorkListUiBean uiBean = SWAKInjector.getInstance(SinkeiyakuWorkListUiBean.class); %>
<script type="text/javascript"><!--
$(function(){
$(<%= SKEIJIMUKBN_SPAN %>).html($(<%= SKEIJIMUKBN_SPAN %>).html().replace(/&nbsp;/ig,""));
$(<%= MOSUKETUKEKBN_SPAN %>).html($(<%= MOSUKETUKEKBN_SPAN %>).html().replace(/&nbsp;/ig,""));
  $(".messagesItem").each(
  function(){
    <% if(uiBean.getPageNo() == PAGENO_INPUTKEYTSKSNTKSNKYK){ %>
        if ($(<%= RESULTINFOTSKSNTKBASE %>)[0].rows.length <= 1) {
            $(<%= RESULTINFOTSKSNTKBASE %>).visible(false);
        }
    <% } %>

    <% if(uiBean.getPageNo() == PAGENO_INPUTKEYANKNITRNSNKYK){ %>
