...
/**
 * 目標額到達時円建変更受付 : 画面UiBeanの抽象親クラスです。<br />
 * このソースはツールによる自動生成で、画面項目定義の変更に連動して更新されます。
 * 手書きコードを加えないでください。
 */
public abstract class GenKhMkhgkTttYendtHnkUktkUiBean extends AbstractUiBean {
    ...
    @ValidDate
    private BizDate syoukaiymdfrom;

    public BizDate getSyoukaiymdfrom() {
        return syoukaiymdfrom;
    }

    @Trim("both")
    @DataConvert("toSingleByte")
    public void setSyoukaiymdfrom(BizDate pSyoukaiymdfrom) {
        syoukaiymdfrom = pSyoukaiymdfrom;
    }

    @ValidDate
    private BizDate syoukaiymdto;

    public BizDate getSyoukaiymdto() {
        return syoukaiymdto;
    }

    @Trim("both")
    @DataConvert("toSingleByte")
    public void setSyoukaiymdto(BizDate pSyoukaiymdto) {
        syoukaiymdto = pSyoukaiymdto;
    }
    ...
    private BizCurrency yendthnkhrgk;
