...
import yuyu.def.db.id.PKBM_ChkTuusanS;
import yuyu.def.db.meta.GenQBM_ChkTuusanS;
import yuyu.def.db.meta.QBM_ChkTuusanS;

/**
 * 通算限度Ｓチェックマスタ テーブルのマッピング情報クラスで、 {@link BM_ChkTuusanS} のスーパークラスです。<br />
 * <table border="1" cellspacing="0" cellpadding="0">
 *  <tr bgcolor="whitesmoke"><th colspan="2">テーブルID</th><th colspan="3">名称</th></tr>
 *  <tr><td colspan="2">{@link BM_ChkTuusanS}</td><td colspan="3">通算限度Ｓチェックマスタ</td></tr>
 *  <tr bgcolor="whitesmoke"><th>フィールドID</th><th>フィールド名</th><th>PKey</th><th>DB型</th><th>Java型</th></tr>
 *  <tr><td>{@link #getHhknfromnen hhknfromnen}</td><td>被保険者年齢自</td><td align="center">{@link PKBM_ChkTuusanS ○}</td><td align="center">N</td><td>BizNumber</td></tr>
 *  <tr><td>{@link #getHhkntonen hhkntonen}</td><td>被保険者年齢至</td><td align="center">{@link PKBM_ChkTuusanS ○}</td><td align="center">N</td><td>BizNumber</td></tr>
 ...
 */
@MappedSuperclass
@Table(name=GenBM_ChkTuusanS.TABLE_NAME)
@IdClass(value=PKBM_ChkTuusanS.class)
@TypeDefs({
    @TypeDef(name="BizCurrencyType", typeClass=BizCurrencyType.class),
    @TypeDef(name="BizNumberType", typeClass=BizNumberType.class)
})
public abstract class GenBM_ChkTuusanS extends AbstractExDBEntity<BM_ChkTuusanS, PKBM_ChkTuusanS> {

    private static final long serialVersionUID = 1L;

    public static final String TABLE_NAME               = "BM_ChkTuusanS";
    ...
    private final PKBM_ChkTuusanS primaryKey;

    public GenBM_ChkTuusanS() {
        primaryKey = new PKBM_ChkTuusanS();
    }

    public GenBM_ChkTuusanS(BizNumber pHhknfromnen, BizNumber pHhkntonen) {
        primaryKey = new PKBM_ChkTuusanS(pHhknfromnen, pHhkntonen);
    }

    @Transient
    @Override
    public PKBM_ChkTuusanS getPrimaryKey() {
        return primaryKey;
    }

    @Transient
    @Override
    public Class<QBM_ChkTuusanS> getMetaClass() {
        return QBM_ChkTuusanS.class;
    }

    @Id
    @Type(type="BizNumberType")
    @Column(name=GenBM_ChkTuusanS.HHKNFROMNEN)
    ...
    @Id
    @Type(type="BizNumberType")
    @Column(name=GenBM_ChkTuusanS.HHKNTONEN)
