...
import yuyu.def.db.id.PKIT_BAK_HengakuUnit;
...
/**
 * 変額ユニットバックアップテーブル テーブルのマッピング情報クラスで、 {@link IT_BAK_HengakuUnit} のスーパークラスです。<br />
 * <table border="1" cellspacing="0" cellpadding="0">
 *  <tr bgcolor="whitesmoke"><th colspan="2">テーブルID</th><th colspan="3">名称</th></tr>
 *  <tr><td colspan="2">{@link IT_BAK_HengakuUnit}</td><td colspan="3">変額ユニットバックアップテーブル</td></tr>
 *  <tr bgcolor="whitesmoke"><th>フィールドID</th><th>フィールド名</th><th>PKey</th><th>DB型</th><th>Java型</th></tr>
 *  <tr><td>{@link #getKbnkey kbnkey}</td><td>区分キー</td><td align="center">{@link PKIT_BAK_HengakuUnit ○}</td><td align="center">V</td><td>String</td></tr>
 *  <tr><td>{@link #getSyono syono}</td><td>証券番号</td><td align="center">{@link PKIT_BAK_HengakuUnit ○}</td><td align="center">V</td><td>String</td></tr>
 *  <tr><td>{@link #getTrkssikibetukey trkssikibetukey}</td><td>取消識別キー</td><td align="center">{@link PKIT_BAK_HengakuUnit ○}</td><td align="center">V</td><td>String</td></tr>
 *  <tr><td>{@link #getUnitfundkbn unitfundkbn}</td><td>ユニットファンド区分</td><td align="center">{@link PKIT_BAK_HengakuUnit ○}</td><td align="center">V</td><td>{@link C_UnitFundKbn}</td></tr>
 ...
 * @see     PKIT_BAK_HengakuUnit
 * @see     QIT_BAK_HengakuUnit
 * @see     GenQIT_BAK_HengakuUnit
 */
@MappedSuperclass
@Table(name=GenIT_BAK_HengakuUnit.TABLE_NAME)
@IdClass(value=PKIT_BAK_HengakuUnit.class)
@TypeDefs({
    @TypeDef(name="BizDateType", typeClass=BizDateType.class),
    @TypeDef(name="BizNumberType", typeClass=BizNumberType.class),
    @TypeDef(name="UserType_C_UnitFundKbn", typeClass=UserType_C_UnitFundKbn.class)
})
public abstract class GenIT_BAK_HengakuUnit extends AbstractExDBEntity<IT_BAK_HengakuUnit, PKIT_BAK_HengakuUnit> {

    private static final long serialVersionUID = 1L;

    public static final String TABLE_NAME               = "IT_BAK_HengakuUnit";
    ...
    private final PKIT_BAK_HengakuUnit primaryKey;

    public GenIT_BAK_HengakuUnit() {
        primaryKey = new PKIT_BAK_HengakuUnit();
    }
    ...
    @Transient
    @Override
    public PKIT_BAK_HengakuUnit getPrimaryKey() {
        return primaryKey;
    }

    @Transient
    @Override
    public Class<QIT_BAK_HengakuUnit> getMetaClass() {
        return QIT_BAK_HengakuUnit.class;
    }

    @Id
    @Column(name=GenIT_BAK_HengakuUnit.KBNKEY)
    ...
    @Id
    @Column(name=GenIT_BAK_HengakuUnit.SYONO)
    @SyoukenNo
    ...
    @Id
    @Column(name=GenIT_BAK_HengakuUnit.TRKSSIKIBETUKEY)
    ...
    @Column(name=GenIT_BAK_HengakuUnit.UNITFUNDKBN)
    ...
    @Column(name=GenIT_BAK_HengakuUnit.HENKOUSIKIBETUKEY)
    ...
    @Column(name=GenIT_BAK_HengakuUnit.UNITKOURYOKUHASSEIYMD)
    ...
    @Type(type="BizDateType")
    @Column(name=GenIT_BAK_HengakuUnit.UNITKOURYOKUENDYMD)
    ...
    @Column(name=GenIT_BAK_HengakuUnit.UNITSUU)
    ...
    @Column(name=GenIT_BAK_HengakuUnit.FUNDHAIBUNWARIAI)
    ...
    @Column(name=GenIT_BAK_HengakuUnit.GYOUMUKOUSINKINOU)
    ...
    @Column(name=GenIT_BAK_HengakuUnit.GYOUMUKOUSINSYAID)
    ...
    @Column(name=GenIT_BAK_HengakuUnit.GYOUMUKOUSINTIME)
