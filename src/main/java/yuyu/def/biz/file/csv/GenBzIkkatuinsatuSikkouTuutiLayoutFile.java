...
/**
 * （共通）専用帳票一括印刷送信ファイル（失効通知レコード）レイアウトファイル データレイアウトのベースBeanクラスです。<br />
 * このソースはツールにより自動生成されたものです。データレイアウト定義に変更があると上書きされてしまいます。
 */
public class GenBzIkkatuinsatuSikkouTuutiLayoutFile implements Serializable {

    private static final long serialVersionUID = 1L;

    public GenBzIkkatuinsatuSikkouTuutiLayoutFile() {
    }
    ...
    @Column(order = 17, label = "（ＩＦｃ）失効日")
    private BizDate ifcSikkouymd;

    public BizDate getIfcSikkouymd() {
        return ifcSikkouymd;
    }

    public void setIfcSikkouymd(BizDate pIfcSikkouymd) {
        ifcSikkouymd = pIfcSikkouymd;
    }

    @Column(order = 18, label = "（ＩＦｃ）取扱コード")
    ...
    @Column(order = 19, label = "（ＩＦｃ）帳票コード")
    ...
    @Column(order = 20, label = "（ＩＦｃ）帳票連絡先識別区分")
