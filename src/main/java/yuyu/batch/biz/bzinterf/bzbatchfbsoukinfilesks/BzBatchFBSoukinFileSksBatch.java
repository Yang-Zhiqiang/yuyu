...
/**
 * 業務共通 インターフェイス バッチＦＢ送金ファイル作成
 */
public class BzBatchFBSoukinFileSksBatch implements Batch {

    @Inject
    private BatchLogger batchLogger;

    @Inject
    private BzBatchFBSoukinFileSksBatchParam bzBatchParam;
    ...
    public void execute() {
        ...
        List<BzFbsoukinFixedlengthRecordLayoutFile> bzFbsoukinFixedlengthRecordLayoutFileLst =
            bzFbSoukinFileSks.creadFBsoukinfwairu(
                bzBatchParam.getSyoriYmd(), bzBatchParam.getIbKakutyoujobcd(), C_FbSoukinBatchKbn.BATCH,
                C_ShrhousiteiKbn.FB_3EIGYOUBI, C_ShrhousiteiKbn.FB_SOUDAIKAI_YOKUEI);
        String batchFileOut = FilePathUtil.getBatchOutDataPath();

        String filePath = batchFileOut + "/" + bzBatchParam.getFileNm3eiatoTyakkin();
        ...
        try {
            ...
            throw new BizAppException(e, MessageUtil.getMessage(MessageId.EBF0163,
                bzBatchParam.getFileNm3eiatoTyakkin()));
        }


        int count = bzFbSoukinFileSks.getDateCount();
