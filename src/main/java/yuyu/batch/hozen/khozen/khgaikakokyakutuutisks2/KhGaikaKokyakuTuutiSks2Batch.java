...
import yuyu.batch.hozen.khozen.khgaikakokyakutuutisks2.dba.KhHenkouUktkBean;
import yuyu.batch.hozen.khozen.khgaikakokyakutuutisks2.dba.KhGaikaKokyakuTuutiSks2Dao;
...
/**
 * 契約保全 契約保全 外貨建顧客通知作成２
 */
public class KhGaikaKokyakuTuutiSks2Batch implements Batch {
    ...
    @Inject
    private KhGaikaKokyakuTuutiSks2BatchParam khGaikaKokyakuTuutiSks2BatchParam;

    @Inject
    private HozenDomManager hozenDomManager;

    @Inject
    private KhGaikaKokyakuTuutiSks2Dao khGaikaKokyakuTuutiSks2Dao;
    ...
    public BatchParam getParam() {
        return khGaikaKokyakuTuutiSks2BatchParam;
    }
    ...
    public void execute() {
        ...
        BizDate syoriYmd = khGaikaKokyakuTuutiSks2BatchParam.getSyoriYmd();
        ...
        CheckTyohyouHnbnKknKokyakuTuutiSks checkTyohyouHnbnKknKokyakuTuutiSks =
            SWAKInjector.getInstance(CheckTyohyouHnbnKknKokyakuTuutiSks.class);

        HashMap<String, Boolean> hnbnKknnFlgMap= new HashMap<>();
        HashMap<String, Integer> hnbnKknnCntMap= new HashMap<>();

        String ibKakutyoujobCd = khGaikaKokyakuTuutiSks2BatchParam.getIbKakutyoujobcd();
        int syoriKensuu = 0;
        ...
        try (ExDBResults<GaikaKokyakuTuutiInfosByKakutyoujobcdBean> exDBResults = khGaikaKokyakuTuutiSks2Dao.
            ...
            for (GaikaKokyakuTuutiInfosByKakutyoujobcdBean gaikaKokyakuTuutiInfosByKakutyoujobcdBean : exDBResults) {
                ...
                if (syohinHanteiKbn == SyouhinUtil.YOTEIRIRITUHENDOU_GONENRISAHAITOU_SITEITUUKA_NENKIN) {
                    syrYmdHanteiKekka= checkSyrTishYm(kykSyouhn.getKykymd(),C_Hrkkaisuu.NEN ,syoriYmd);

                    if(syrYmdHanteiKekka.containsKey(true)){
                        syouhinHanmeiKbn = EditKykNaiyouOsirase.SYOUHNHANMEIKBN_DUO;
                        kykOutouYMD = syoriYmd;
                    }else{
                        continue;
                    }
                }else{
                ...
                List<JT_SkKihon> skKihonList =
                    khGaikaKokyakuTuutiSks2Dao.getSkKihonsBySyonoSeikyuusyubetuTorikesiflg(
                ...
                List<KhHenkouUktkBean> khHenkouUktkBeanLst = khGaikaKokyakuTuutiSks2Dao.getKhHenkouUktkBySyono(kykKihon
                    .getSyono());
                ...
                ZT_GaikaKokyakuTuutiTy gaikaKokyakuTuutiTy = editKykNaiyouOsirase.editInfo(
                    ...
                    kykOutouYMD,
                    syouhinHanmeiKbn,
                    khGaikaKokyakuTuutiSks2BatchParam.getCampaignUmuKbn(),
                    null,
                ...
                HanteiKekkaKokyakuTuutiSksBean hanteiKekkaBean = checkTyohyouHnbnKknKokyakuTuutiSks.exec(
                    khTyouhyouHonbanHknList, hnbnKknnFlgMap, kykKihon, kykSyouhn, gaikaKokyakuTuutiTy);
                HanteiKekkaKokyakuTuutiSksBean hanteiKekkaKensuBean = checkTyohyouHnbnKknKokyakuTuutiSks.hanteiByCount(
                ...
                if (C_UmuKbn.ARI.eq(hanteiKekkaBean.getUpdateKbn())||
                    C_UmuKbn.ARI.eq(hanteiKekkaKensuBean.getUpdateKbn())) {
                    ...
                    if (hanteiKekkaKensuBean.getTyohyouHonbanKakuninBeanList() != null &&
                        hanteiKekkaKensuBean.getTyohyouHonbanKakuninBeanList().size() > 0) {
                        khTyouhyouHonbanUbdList.addAll(hanteiKekkaKensuBean.getTyohyouHonbanKakuninBeanList());
                    }
                    hnbnKknnFlgMap = hanteiKekkaBean.getTyouhyouHnbnKknnFlgMap();
                    hnbnKknnCntMap = hanteiKekkaKensuBean.getTyouhyouHnbnKknnCntMap();
                }
    ...
    private HashMap<Boolean,BizDate> checkSyrTishYm(BizDate pKhKijyunYmd,C_Hrkkaisuu pKhHrkkaisuu, BizDate pKhHanteiYmd) {

        logger.debug("▽ 処理対象日付判定 開始  基準日（契約日）：" + pKhKijyunYmd + ",判定日：" + pKhHanteiYmd);
        ...
        if (!keiyakuOutoubi.equals(pKhKijyunYmd) &&
            BizDateUtil.compareYm(keiyakuOutoubi.getBizDateYM(), pKhHanteiYmd.getBizDateYM()) == BizDateUtil.COMPARE_EQUAL) {
            syrTisyuYmdHanteiKekka = true;
        }

        syrYmdHanteiKekka.put(syrTisyuYmdHanteiKekka,keiyakuOutoubi);

        logger.debug("△ 処理対象日付判定 終了  判定結果：" + syrTisyuYmdHanteiKekka + ",応当日：" + keiyakuOutoubi);
