package yuyu.def.db.entity;

import java.util.List;

import javax.inject.Inject;
import javax.transaction.Transactional;

import jp.co.slcs.swak.core.inject.SWAKInjector;
import jp.co.slcs.swak.core.test.SWAKTestRunner;

import org.junit.AfterClass;
import org.junit.Assert;
import org.junit.BeforeClass;
import org.junit.Test;
import org.junit.runner.RunWith;

import yuyu.def.classification.C_KyhgndkataKbn;
import yuyu.def.siharai.manager.SiharaiDomManager;
import yuyu.testinfr.TestOrder;

/**
 * 支払商品属性マスタ エンティティのテストクラス<br />
 */
@RunWith(SWAKTestRunner.class)
public class JM_SiSyouhnZokuseiEntityTest implements EntityTestInterface {

    @Inject
    private SiharaiDomManager siharaiDomManager;

    private static final String pSyouhncd1 = "1111";
    private static final C_KyhgndkataKbn pKyhgndkatakbn1 = C_KyhgndkataKbn.TYPE60;
    private static final Integer pSyouhnsdno1 = 11;
    private static final String pKyuuhucd1 = "1111";
    private static final String pSyouhncd2 = "2222";
    private static final C_KyhgndkataKbn pKyhgndkatakbn2 = C_KyhgndkataKbn.TYPE60;
    private static final Integer pSyouhnsdno2 = 22;
    private static final String pKyuuhucd2 = "2222";
    //    private static final String pSyouhncd3 = "3333";
    //    private static final C_KyhgndkataKbn pKyhgndkatakbn3 = C_KyhgndkataKbn.TYPE60;
    //    private static final Integer pSyouhnsdno3 = 33;
    //    private static final String pKyuuhucd3 = "3333";

    @BeforeClass
    @Transactional
    public static void insertTestData() {

        SiharaiDomManager siharaiDomManager = SWAKInjector.getInstance(SiharaiDomManager.class);


        JM_SiSyouhnZokusei jM_SiSyouhnZokusei2 = new JM_SiSyouhnZokusei();
        jM_SiSyouhnZokusei2.setSyouhncd(pSyouhncd2);
        jM_SiSyouhnZokusei2.setKyhgndkatakbn(pKyhgndkatakbn2);
        jM_SiSyouhnZokusei2.setSyouhnsdno(pSyouhnsdno2);
        jM_SiSyouhnZokusei2.setKyuuhucd(pKyuuhucd2);

        siharaiDomManager.insert(jM_SiSyouhnZokusei2);

        JM_SiSyouhnZokusei jM_SiSyouhnZokusei3 = new JM_SiSyouhnZokusei();
        jM_SiSyouhnZokusei3.setSyouhncd(pSyouhncd1);
        jM_SiSyouhnZokusei3.setKyhgndkatakbn(pKyhgndkatakbn2);
        jM_SiSyouhnZokusei3.setSyouhnsdno(pSyouhnsdno2);
        jM_SiSyouhnZokusei3.setKyuuhucd(pKyuuhucd1);

        siharaiDomManager.insert(jM_SiSyouhnZokusei3);

        JM_SiSyouhnZokusei jM_SiSyouhnZokusei1 = new JM_SiSyouhnZokusei();
        jM_SiSyouhnZokusei1.setSyouhncd(pSyouhncd1);
        jM_SiSyouhnZokusei1.setKyhgndkatakbn(pKyhgndkatakbn1);
        jM_SiSyouhnZokusei1.setSyouhnsdno(pSyouhnsdno1);
        jM_SiSyouhnZokusei1.setKyuuhucd(pKyuuhucd1);

        siharaiDomManager.insert(jM_SiSyouhnZokusei1);

        JM_Kyuuhu jM_Kyuuhu1 = new JM_Kyuuhu(pKyuuhucd1);
        siharaiDomManager.insert(jM_Kyuuhu1);

    }

    @AfterClass
    @Transactional
    public static void deleteTestData() {

        SiharaiDomManager siharaiDomManager = SWAKInjector.getInstance(SiharaiDomManager.class);

        JM_SiSyouhnZokusei jM_SiSyouhnZokusei1 = siharaiDomManager.getSiSyouhnZokusei(pSyouhncd1, pKyhgndkatakbn1, pSyouhnsdno1, pKyuuhucd1);

        siharaiDomManager.delete(jM_SiSyouhnZokusei1);

        JM_SiSyouhnZokusei jM_SiSyouhnZokusei2 = siharaiDomManager.getSiSyouhnZokusei(pSyouhncd2, pKyhgndkatakbn2, pSyouhnsdno2, pKyuuhucd2);

        siharaiDomManager.delete(jM_SiSyouhnZokusei2);

        JM_SiSyouhnZokusei jM_SiSyouhnZokusei3 = siharaiDomManager.getSiSyouhnZokusei(pSyouhncd1, pKyhgndkatakbn2, pSyouhnsdno2, pKyuuhucd1);

        siharaiDomManager.delete(jM_SiSyouhnZokusei3);
    }

    @Override
    @Test
    @TestOrder(10)
    public void getAll() {

        List<JM_SiSyouhnZokusei> jM_SiSyouhnZokuseis = siharaiDomManager.getAllSiSyouhnZokusei();

        Assert.assertEquals(3, jM_SiSyouhnZokuseis.size());


        Assert.assertEquals(jM_SiSyouhnZokuseis.get(0), siharaiDomManager.getSiSyouhnZokusei(pSyouhncd1, pKyhgndkatakbn1, pSyouhnsdno1, pKyuuhucd1));
        Assert.assertEquals(jM_SiSyouhnZokuseis.get(1), siharaiDomManager.getSiSyouhnZokusei(pSyouhncd1, pKyhgndkatakbn2, pSyouhnsdno2, pKyuuhucd1));
        Assert.assertEquals(jM_SiSyouhnZokuseis.get(2), siharaiDomManager.getSiSyouhnZokusei(pSyouhncd2, pKyhgndkatakbn2, pSyouhnsdno2, pKyuuhucd2));
    }

    @Override
    @Test
    @TestOrder(20)
    public void getUniqueEntity() {

        JM_SiSyouhnZokusei jM_SiSyouhnZokusei1 = siharaiDomManager.getSiSyouhnZokusei(pSyouhncd1, pKyhgndkatakbn1, pSyouhnsdno1, pKyuuhucd1);

        Assert.assertEquals(pSyouhncd1, jM_SiSyouhnZokusei1.getSyouhncd());
        Assert.assertEquals(pKyhgndkatakbn1, jM_SiSyouhnZokusei1.getKyhgndkatakbn());
        Assert.assertEquals(pSyouhnsdno1, jM_SiSyouhnZokusei1.getSyouhnsdno());
        Assert.assertEquals(pKyuuhucd1, jM_SiSyouhnZokusei1.getKyuuhucd());
    }

    @Override
    @Test
    @TestOrder(30)
    public void noUniqueEntity() {

        String pSyouhncd9 = "9999";
        C_KyhgndkataKbn pKyhgndkatakbn9 = C_KyhgndkataKbn.TYPE60;
        Integer pSyouhnsdno9 = 99;
        String pKyuuhucd9 = "9999";
        JM_SiSyouhnZokusei jM_SiSyouhnZokusei1 = siharaiDomManager.getSiSyouhnZokusei(pSyouhncd9, pKyhgndkatakbn9, pSyouhnsdno9, pKyuuhucd9);

        Assert.assertNull(jM_SiSyouhnZokusei1);
    }

    @Override
    @Test
    @TestOrder(40)
    public void blankCondition() {

        String pSyouhncdBlank = "";
        String pKyuuhucdBlank = "";
        JM_SiSyouhnZokusei jM_SiSyouhnZokusei1 = siharaiDomManager.getSiSyouhnZokusei(pSyouhncdBlank, pKyhgndkatakbn1, pSyouhnsdno1, pKyuuhucdBlank);

        Assert.assertNull(jM_SiSyouhnZokusei1);
    }

    @Override
    @Test
    @TestOrder(50)
    public void nullCondition() {
        Assert.assertTrue(true);
    }

    @Test
    @TestOrder(60)
    public void getKyuuhu() {

        JM_SiSyouhnZokusei jM_SiSyouhnZokusei1 = siharaiDomManager.getSiSyouhnZokusei(pSyouhncd1, pKyhgndkatakbn1, pSyouhnsdno1, pKyuuhucd1);
        JM_Kyuuhu jM_Kyuuhu1 = jM_SiSyouhnZokusei1.getKyuuhu();

        Assert.assertEquals(pKyuuhucd1, jM_Kyuuhu1.getKyuuhucd());

        JM_SiSyouhnZokusei jM_SiSyouhnZokusei2 = siharaiDomManager.getSiSyouhnZokusei(pSyouhncd2, pKyhgndkatakbn2, pSyouhnsdno2, pKyuuhucd2);
        JM_Kyuuhu jM_Kyuuhu2 = jM_SiSyouhnZokusei2.getKyuuhu();
        JM_SiSyouhnZokusei jM_SiSyouhnZokusei3 = siharaiDomManager.getSiSyouhnZokusei(pSyouhncd1, pKyhgndkatakbn2, pSyouhnsdno2, pKyuuhucd1);
        JM_Kyuuhu jM_Kyuuhu3 = jM_SiSyouhnZokusei3.getKyuuhu();
        Assert.assertNull(jM_Kyuuhu2);
        Assert.assertEquals(pKyuuhucd1, jM_Kyuuhu3.getKyuuhucd());
    }

    @Test
    @TestOrder(70)
    @Transactional
    public void getKyuuhuNull01() {

        JM_Kyuuhu jM_Kyuuhu1 = siharaiDomManager.getKyuuhu(pKyuuhucd1);

        if(jM_Kyuuhu1 != null) {
            siharaiDomManager.delete(jM_Kyuuhu1);
        }
    }

    @Test
    @TestOrder(80)
    @Transactional
    public void getKyuuhuNull02() {

        JM_SiSyouhnZokusei jM_SiSyouhnZokusei1 = siharaiDomManager.getSiSyouhnZokusei(pSyouhncd1, pKyhgndkatakbn1, pSyouhnsdno1, pKyuuhucd1);
        JM_Kyuuhu jM_Kyuuhu1 = jM_SiSyouhnZokusei1.getKyuuhu();

        Assert.assertNull(jM_Kyuuhu1);
    }
}
