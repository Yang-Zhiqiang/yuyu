...
/**
 * 不備管理チェッククラスのメソッド {@link CheckHubiKanri#exec(MosnaiCheckParam)} テスト用クラスです。<br />
 */
@RunWith(OrderedRunner.class)
public class CheckHubiKanriTest_exec {

    private final static String fileName = "UT_SP_単体テスト仕様書_不備管理チェック";
    ...
    public void testExec_A1() {
        ...
        checkHubiKanri.exec(mosnaiCheckParam);

        syoriCTL = sinkeiyakuDomManager.getSyoriCTL(mosNo1);
        List<HT_HubiMsg> hubiMsg = syoriCTL.getHubiMsgs();

        exStringEquals(String.valueOf(hubiMsg.size()), "0", "不備メッセージテーブルの件数");
    ...
    public void testExec_B1() {
        ...
        exStringEquals(hubiMsg.get(0).getMessageId(), "EHC0160", "不備メッセージID");
        exStringEquals(hubiMsg.get(0).getHubimsg(), "未解消の不備登録情報があります。", "不備メッセージ");
