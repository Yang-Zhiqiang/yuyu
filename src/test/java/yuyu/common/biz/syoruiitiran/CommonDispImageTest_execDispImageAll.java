...
/**
 * 業務共通 書類情報一覧 イメージ表示共通のメソッド {@link CommonDispImage#execDispImageAll(String[])} テスト用クラスです。<br />
 */
@RunWith(OrderedRunner.class)
public class CommonDispImageTest_execDispImageAll {
    ...
    @TestOrder(10)
    public void testExecDispImageAll_A1() {
        MockObjectManager.initialize();
        IwfImageClientMockForBiz.mode = "dl01-01";
        String pImageid[] = {"10010","10020","10030"};

        baseUserInfo.getUser().setUserId("JUnit");

        try {

            commonDispImage.execDispImageAll(pImageid);
            ...
            exNumericEquals(getImageInBean.getImageids().length, 3, "イメージＩＤ配列の件数");
            exStringEquals(getImageInBean.getImageids()[0], "10010", "イメージＩＤ配列");
            exStringEquals(getImageInBean.getImageids()[1], "10020", "イメージＩＤ配列");
            exStringEquals(getImageInBean.getImageids()[2], "10030", "イメージＩＤ配列");
    ...
    @TestOrder(20)
    public void testExecDispImageAll_A2() {
        MockObjectManager.initialize();
