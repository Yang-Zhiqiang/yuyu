package yuyu.common.siharai.chksk;

import java.util.List;

import yuyu.def.classification.C_SouhusakiKbn;
import yuyu.def.db.entity.JT_SiKykKihon;

/**
 * {@link ChkSubShskInfo}のモッククラスです。<br />
 */
public class ChkSubShskInfoMockForSiharai extends ChkSubShskInfo {

    public static final String TESTPATTERN1 = "1";

    public static final String TESTPATTERN2 = "2";

    public static final String TESTPATTERN3 = "3";

    public static Class<?> caller = null;

    public static String SYORIPTN = null;

    @Override
    public void exec(C_SouhusakiKbn pSouhusakiKbn, String pTsinyNo, String pTsinadr1kj, String pTsinadr2kj, String
        pTsinadr3kj, String pTsintelNo, JT_SiKykKihon pSiKykKihon, List<ChkKekkaBean> pChkKekkaBeanLst) {

        if (CheckSbSkNaiyouTest_exec.class == caller) {
            if (TESTPATTERN1.equals(SYORIPTN)) {
                return;
            }
        }
        super.exec(pSouhusakiKbn, pTsinyNo, pTsinadr1kj, pTsinadr2kj, pTsinadr3kj, pTsintelNo, pSiKykKihon,
            pChkKekkaBeanLst);

    }

}
