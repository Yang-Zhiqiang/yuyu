...
##############################################################################
###
### ■使用目的       ： 予定利率連動ファイル_HULFT起動
###
### ■使用方法       ： $ <シェル名> 引数（11個）
###                     #1～10 「ED-IF_インターフェース概要書_予定利率連動Ｆ.xlsx」参照
...
BK_FILE=/usr/dbif/batch_out/A8ZRG241Z*
if [ -e ${BK_FILE} ]; then
   rm -f ${BK_FILE}
fi
...
# ファイルをコピーする
cp -p /usr/dbif/batch_out/BM_YoteiRiritu_rendou.csv ${IN_FILE_NAME}
