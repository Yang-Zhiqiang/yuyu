...
### ■製作者／改変者　　　：SLC
###
### ■使用目的　　　　　　：ホストからのオンライン開始信号の受信（ＤＳインスタンス）
###
### ■使用方法　　　　　　：第一引数：ポート番号
###                                       社内（ray）     : 9080
###                                       代理店（ray_ag）: 9083
...
RESPONSE=`curl -s http://localhost:9082/ray_ds/webservice/yuyu.infrext.mq.MqControlSignalReceiveService/execMqOnlineStartSignalReceive -o /dev/null -w "%{http_code}\n"`
